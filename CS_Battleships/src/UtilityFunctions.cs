...
sealed class UtilityFunctions
{
    ...
    private readonly static Color SMALL_SEA = SwinGame.RGBAColor((byte) 6, (byte) 60, (byte) 94, (byte) 255);
    private readonly static Color SMALL_SHIP = Color.Gray;
    private readonly static Color SMALL_MISS = SwinGame.RGBAColor((byte) 1, (byte) 147, (byte) 220, (byte) 255);
    private readonly static Color SMALL_HIT = SwinGame.RGBAColor((byte) 169, (byte) 24, (byte) 37, (byte) 255);
    ...
    private readonly static Color MESSAGE_COLOR = SwinGame.RGBAColor((byte) 2, (byte) 167, (byte) 252, (byte) 255);
    ...
    private static void DrawCustomField(ISeaGrid grid, Player thePlayer, bool small, bool showShips, int left, int top, int width, int height, int cellWidth, int cellHeight, int cellGap)
    {
        ...
        foreach (Ship s in thePlayer)
        {
            ...
            {
                shipName = "ShipLR" + System.Convert.ToString(s.Size);
            ...
            {
                //Up down
                shipName = "ShipUD" + System.Convert.ToString(s.Size);
    ...
    /*L: statement repeats a group of embedded statements which is UpdateSprite for each element in an array or object,
     * then, if condition (s.animationHasEnded) is true then , it will cal ended.Add)
     */
    public static void UpdateAnimations()
    {
        ...
        {
            //L: FreeSprite frees a sprite, this does not free the sprite's bitmaps, which allows bitmaps to be shared between sprites. All created sprites need to be freed.
            _Animations.Remove(s);
            SwinGame.FreeSprite(s);
        }
    }

    // L: create a animaiton
    ...
    //L: If i less or equal than (animation_cells * frames_per_cell), it will add 1 to itself and call UpdateAnimations to draw animation
